...
var ch = new channel();	
	...
	function verCotizacion(fileno) {
		...
		<%	else	%>
		alert("<% =GF_TRADUCIR("Aún no se ha cerrado el plazo de presentación de cotizaciones.") %>");
		<%	end if %>
	}
			...
			while (not rsCotizaciones.eof)	
				...
				if (isNull(rsCotizaciones("CDUSRAPERTURA")) and pct_tipoCompra = TIPO_PCT_CONCURSO) then				
					...
					if (rsCotizaciones("PATHCOTIZACION") <> ACCION_PCT_RETIRARSE) then
						...
						textoFechaApertura = "<B>" & GF_TRADUCIR("El proveedor decidió no participar de la cotizacón.") & "</B>"
						mensaje = "alert('El proveedor decidió no participar de la cotizacón.');"
						declino = true
					end if
				else		
					if (rsCotizaciones("PATHCOTIZACION") <> ACCION_PCT_RETIRARSE) then		
