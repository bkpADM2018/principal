				...
				while not rsLista.eof  and (reg < p_mostrar) 
					...
					<tr>						
						...
						<%  'Si es un estado terminal (dependiendo el circuito del camion) y tiene kilos netos se supone que termino OK el ciruito
							...
							<img src="../images/pdf-16.png" title="Ver Nota de recepción" onclick="abrirNotaRecepcion('<%=Trim(rsLista("IDCAMION"))%>','<%=Trim(rsLista("NUCARTAPORTE"))%>','<%=dtContable%>',<%=rsLista("CIRCUITO") %>)" style="cursor:pointer;">
					    <%  end if %>
						</td>
                        <td  align="center">
