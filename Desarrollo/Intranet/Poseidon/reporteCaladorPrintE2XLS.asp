...
Function imprimirFiltros()
	Dim auxCoordinador, auxCordinado, auxProducto, auxFechaDesdeD,auxFechaDesde,auxCalador,auxChkResumen  %>
    ...
    <%  auxChkResumen = "NO"
        if (g_chkResumen = ESTADO_ACTIVO) then auxChkResumen = "SI" %>>
            <td colspan="2" align="left"><%=GF_TRADUCIR("Solo Resumen")%></td>
            <td colspan="4" align="left"><% =auxChkResumen %></td>
		</tr> 
		<tr>
	<%	auxUsuario = "Todos"
...
Function drawTitulosDetalle()
    if (g_chkResumen <> ESTADO_ACTIVO) then %>	
	...
	Dim myField, h, rtrn,myRegistro,columnInit,auxCdRubro,auxVlRubro 		
		...
		<%		end if	
                    For z = 0 To UBound(myField)
						...
						if (z < INDEX_DSRUBRO) AND (g_chkResumen <> ESTADO_ACTIVO)then %>
...
'*****************************************************************************************
'	COMIENZO DE PAGINA
'   ETAPA 2 XLS - GENERACION DEL EXCEL
'*****************************************************************************************
			...
			<tr>
				...
				<%		if (dicCam.Count > 0) Then
							...
							for each strItem in dicCam.Items
								if (not flagHayResultado) AND (g_chkResumen <> ESTADO_ACTIVO) then Call drawTituloCabecera(arrTitulosCamiones)
			...
			<tr>
				...
				<%		if (dicVag.Count > 0) Then
							...
							for each strItem in dicVag.Items
								if (not flagHayResultado) AND (g_chkResumen <> ESTADO_ACTIVO) then Call drawTituloCabecera(arrTitulosVagones)
								str = Split(strItem, SECTOR_TOKEN)
								strCabecera = str(0)
								strDetalle  = str(1)
								netoSinMerma = drawCabecera(strCabecera)
