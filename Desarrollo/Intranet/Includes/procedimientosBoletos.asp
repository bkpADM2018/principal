...
Function getDataEntity(idEntity, ByRef DsEntity, ByRef ArbEntity, ByRef DicEntity)
	...
	DicEntity = "#Dirección no encontrada#"
...
sub GF_dibujarEncabezado(p_oPDF, p_boleto)
    ...
    Call GF_writeText(p_oPDF, 120, 103, "Código SIO Granos:", 0)
    ...
    call GF_writeTextAlign(p_oPDF, 1, intLineaBase, "modificatorias, y Art. 18 de la correspondiente Reglamentacióm General, Decreto 3666/55 y modificatorias)", 600, PDF_ALIGN_CENTER)
...
sub GF_dibujarClausulas(p_oPDF, p_boleto)
    ...
    select case cint(p_boleto("Operacion"))
        ...
        case 6,9,10,11,12:
            ...
            aclaracionMoneda = "dólares"
    end select

    if p_boleto("CamionesPactados")=1 then strCamiones = "camión"
    strParrafo = "<b>" & intItem & ".-</b> Los señores <b>" & GetDsEnterprise2(p_boleto("KCVEN")) & "</b> (CUIT:" & formatear(p_boleto("VEN_CUIT"),"CUIT") & "), domiciliado en " & getEnterpriseDIR(p_boleto("KCVEN"))& " C.P. " & getEnterpriseCP(p_boleto("KCVEN")) & " " & getEnterpriseLoc(p_boleto("KCVEN")) & " PCIA de " & Trim(getEnterpriseProv(p_boleto("KCVEN"))) & ", venden a <b>" & p_boleto("RSCOMP") & ",</b> domiciliada en Av del Libertador 350 10º PISO, Vicente Lopez (C.P. 1638), la cantidad de <b>" & GF_EDIT_INTEGER(p_boleto("Kilos")) & " (" & trim(NroEnLetras(p_boleto("Kilos"), true)) & ") Kgs "
	if ucase(p_boleto("CodigoPago")) = "X" then strParrafo = strParrafo & ", o el resultante de " & p_boleto("CamionesPactados") & " " & strCamiones & ", "
    strParrafo = strParrafo & "de " & lcase(getDsProduct(clng(p_boleto("Producto")))) & ", y demás condiciones  cámara de la cosecha  <b>" & formatear(clng(p_boleto("Cosecha"))-1,"anio") & "-" & formatear(clng(p_boleto("Cosecha")),"anio")
    ...
    strParrafo = "<b>" & intItem & ".-</b> Las entregas y recibos se efectuarán en <b>Puerto " & getDsPort(p_boleto("PuertoRecepcion")) & "</b> - donde los vendedores deberán remitir la mercadería, en <b>" & lcase(getDsTransport(p_boleto("Transporte"))) & "</b> atracados y descargados desde el  <b>" & GF_FN2DTE(p_boleto("FechaEntDesde")) & "  y  hasta  el  " & GF_FN2DTE(p_boleto("FechaEntHasta")) & "</b>  inclusive, haciéndose el recibo por el recibidor del comprador en destino."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)

    strParrafo = "Procedencia de la Mercadería: <b>" & trim(p_boleto("DSProcedencia"))
    ...
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_LEFT)
    strParrafo = "La mercadería entregada se liquidará según condición <b>CAMARA,</b> salvo las condiciones establecidas en el presente contrato."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_LEFT)
    strParrafo = " Los vendedores declaran en forma expresa que la mercadería: (marque lo que corresponda)"
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_LEFT)
    call GF_dibujarOpcionesMercaderia(p_oPDF, intLineaBase)
	
    intItem = intItem + 1
    intLineaBase = intLineaBase + intSeparacionEntreParrafos    
    strParrafo = "<b>" & intItem & ".-</b> Los gastos correspondientes a servicios de acondicionamiento de la mercadería serán descontados al vendedor o al corredor interviniente, según las tarifas vigentes al momento de la descarga, salvo que sean acreditados al comprador dentro de las 72hs de recibido el servicio. En los contratos a fijar precio, las partes acuerdan que caso de que los servicios de acondicionamiento se realicen previo a la fijación del precio, el comprador podrá fijar el precio, según las condiciones contractuales, únicamente sobre la cantidad de kilogramos entregados necesarios para cancelar los montos equivalentes al acondicionamiento adeudado, salvo que el vendedor acredite al comprador dicho gasto dentro de las 72hs de realizado el servicio."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)
    ...
    intItem = intItem + 1
    intLineaBase = intLineaBase + intSeparacionEntreParrafos
    strParrafo = "<b>" & intItem & ".-</b> El presente contrato es INTRANSFERIBLE. Ninguna de las partes podrá ceder o transferir en forma alguna, ni total o parcialmente, ni el contrato ni los derechos y/u obligaciones emergentes del mismo."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)
    ...
    strParrafo = "<b>" & intItem & ".-</b> Todos los firmantes acuerdan que todas las divergencias, cuestiones o reclamos que surjan de o que se relacionen con cualquiera de las relaciones jurídicas que se deriven de este contrato y entre cualesquiera de ellos, serán resueltas en forma definitiva por la " & strEntidadArbitrante & ". El tribunal actuará como amigable componedor, con aplicación de las Reglas y Usos del Comercio de Granos y del Reglamento de Procedimientos aprobado por Decreto 931/98 y/o sus futuras modificaciones, ampliaciones o normas complementarias."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)
    ...
    strParrafo = "<b>" & intItem & ".-</b> A los efectos del presente boleto las partes constituyen domicilio especial en la " & p_boleto("Entidad_Descripcion") & ", " & direccion & ", donde se notificarán válidamente todas las citaciones, providencias y resoluciones."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)
    ...
    intItem = intItem + 1
    intLineaBase = intLineaBase + intSeparacionEntreParrafos
    strParrafo = "<b>" & intItem & ".-</b> Operación concertada el <b>" & GF_FN2DTE(p_boleto("FechaConc")) & ".</b>"
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_LEFT)
    
    intItem = intItem + 1
    intLineaBase = intLineaBase + intSeparacionEntreParrafos
    strParrafo = "<b>" & intItem & ".-</b> El vendedor deberá cumplir  con la Disposición  General del Servicio Nacional de Sanidad Vegetal Nº  3/83,  sus considerándos y correspondientes Art. 1º ,2º ,3º  que se transcriben a continuación como así también con la los artículos 134 ( Ex 123 ) y Art. 135  ( ex.124 )   de la Ley Provincial de Santa Fe Nro. 10.703 . Disposición 3/83: ARTICULO 1º.- Prohíbase el tratamiento con plaguicida fumigantes de los granos, productos y subproductos de cereales y oleaginosos, durante la carga de los mismos en camiones o vagones y durante el tránsito de éstos hasta su destino.ARTICULO 2º.- Todo camión o vagón en el que se detecten restos de plaguicidas fumigantes sin descomponer o bien concentraciones elevadas de los mismos en el momento de la descarga, serán rechazados, debiendo cumplir o complementar según corresponda, antes de ser descargado, el tiempo de exposición y de ventilación que se indican: 96 horas y 6 horas respectivamente. ARTICULO 3º.- Las demoras y/o perjuicios económicos provocados por el uso inadecuado de estos plaguicidas correrán por cuenta exclusiva de los remitentes."
	intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)    
	...
	if ((p_boleto("Producto") = 17) and (p_boleto("FechaConc") >= "20130320")) then
	    ...
	    if (p_boleto("Cosecha") = 15) then
	        strParrafo = "<b>" & intItem & ".-</b> El vendedor reconoce que la mercadería a ser entregada condice con la variedad de Cebada  Cervecera Declarada en el presente contrato,   la misma  debe mantener la Pureza Varietal al 95% de la declarada, no permitiendo mezclas con otras variedades al momento del almacenaje o carga de la misma.  La Pureza Varietal será determinada por la Cámara Arbitral correspondiente y en caso que  los análisis determinen un valor inferior al 95%, se aplicará una rebaja del 15%."
	    else
	        strParrafo = "<b>" & intItem & ".-</b> El vendedor reconoce que la mercadería a ser entregada condice con la variedad de Cebada  Cervecera Declarada en el presente contrato,   la misma  debe mantener la Pureza Varietal al 98% de la declarada, no permitiendo mezclas con otras variedades al momento del almacenaje o carga de la misma.  La Pureza Varietal será determinada por la Cámara Arbitral correspondiente y en caso que  los análisis determinen un valor inferior al 98%, se aplicará una rebaja del 30%."
        end if	        
        intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)    
	end if
	...
	if p_boleto("FechaConc") > "20120321" then
		intItem = intItem + 1
		intLineaBase = intLineaBase + intSeparacionEntreParrafos
		strParrafo = "<b>" & intItem & ".-</b> El comprador otorgará el cupo con un código alfanumérico que obligatoriamente debe consignarse en el campo observaciones de cada Carta de Porte. En caso de que el vendedor remita camiones sin poseer cupo para la descarga, el comprador podrá, a su exclusiva opción, proceder a la descarga de los mismos, debiendo en tal caso el vendedor abonar al comprador U$S 10 (diez dólares) por tonelada, en concepto de gastos extras por descargas no programadas."
		intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_JUSTIFY)    
	end if
	...
	intLineaBase = intLineaBase + intSeparacionEntreParrafos
    strParrafo = "<b>Observaciones: </b> Destino de la mercadería del Presente Contrato: EXPORTACIÓN. El comprador no actúa como comisionista consignatario en esta operación."
    intLineaBase = GF_writeTextPlus(p_oPDF, 15, intLineaBase, strParrafo, intAnchoLinea, intAltoRenglon, PDF_ALIGN_LEFT)
    ...
    strParrafo = "Queda establecido que toda tasa, contribución, impuesto provincial y/o municipal que grave la presente operación estará a cargo de la parte vendedora."
...
sub armarClausulaCorredor(p_oPDF, p_boleto, byref p_intLineaBase, byref p_intItem, p_intAnchoLinea, p_intAltoRenglon, p_intSeparacionEntreParrafos)
    ...
    if (p_boleto("KCCOR") <> "0") then
        ...
        strTexto = "<b>" & p_intItem & ".-</b> Los señores <b>" & GetDsEnterprise2(p_boleto("KCCOR")) & "</b> actúan en la presente operación como corredores, quedando facultados por los vendedores a endosar en su nombre recibos de mercadería, ampliaciones y/o anulaciones, convenir eventuales prórrogas y a firmar en su nombre y representación toda la documentación necesaria para la instrumentación o formalización del presente y en particular los formularios C 1116 A y B Res. Conj. SAGPyA  nº 456/03 y DGI nº 1593/03"
        ...
        '    p_intItem = p_intItem + 1
        '    p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        '    strTexto =  "<b>" & p_intItem & ".-</b> El vendedor faculta al corredor a firmar en su nombre y representación toda la documentación necesaria para la instrumentación o formalización del presente y en particular los formularios C 1116 A y B Res. Conj. SAGPyA  nº 456/03 y DGI nº 1593/03."
    ...
    if ((cint(p_boleto("Operacion")) = 1) or (cint(p_boleto("Operacion")) = 10)) then
        ...
        p_intItem = p_intItem + 1
        p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        strTexto = "<b>" & p_intItem & ".-</b> El precio de la mercadería objeto del presente contrato, se fijará a partir del <b>" & GF_FN2DTE(p_boleto("FechaFijaDesde")) & "</b> y a mas tardar hasta el <b>" & GF_FN2DTE(p_boleto("FechaFijaHasta")) & ".</b>"
        ...
        p_intItem = p_intItem + 1
        p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        strTexto = "<b>" & p_intItem & ".-</b> Fijaciones mínimas diarias <b>" & p_boleto("KilosMin")/1000 & " TNS</b> y máximas diarias <b>" & p_boleto("KilosMax")/1000 & " TNS.</b>"
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_LEFT)
        
        p_intItem = p_intItem + 1
        p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        strTexto = "<b>" & p_intItem & ".-</b> La Fijación la comunicará el vendedor al comprador el día elegido para la fijación de precio. Dicha comunicación deberá hacerse por medio fehaciente. La fijación de los granos que se comercializarán por el presente contrato se realizará "
        if (Cint(p_boleto("Producto")) = 9) then
			strTexto = strTexto & "por mercado TOEPFER."
        else
			strTexto = strTexto & "en base al que registre La Cámara Arbitral de Cereales. En caso de que el día de la fijación no se registrara cotización pública, las partes podrán solicitar a la Cámara correspondiente la determinación del precio al que habrá de liquidarse la fijación para los casos en que se opte por la pizarra."
        end if
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intAnchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)

        p_intItem = p_intItem + 1
        p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        strTexto = "<b>" & p_intItem & ".-</b> En caso de cesación de pagos, presentación en o declaración de quiebra, o presentación en concurso preventivo de cualquiera de las partes, la otra podrá fijar el precio de la mercadería pendiente de fijación de acuerdo con lo establecido en la cláusula 4ª. A ese efecto, deberá comunicar su resolución a la otra parte, por medio fehaciente, y se procederá de inmediato a la liquidación definitiva de la operación."
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)
    ...
    if cstr(p_boleto("CondicionIVA")) = "C" then
        p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
        p_intItem = p_intItem + 1
        strTexto = "<b>" & p_intItem & ".-</b> El vendedor declara que la mercadería proviene de compraventas con pago en especies, por lo que de acuerdo a lo estipulado por el ART. 6 de la Reg. 2300, el presente contrato no se encuentra sujeto a retención de IVA."
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)
    end if
end sub
'*****************************************************************************************************************
sub armarClausulaFormaDePago(p_oPDF, p_boleto, byref p_intLineaBase, byref p_intItem, p_intAnchoLinea,p_intAltoRenglon, p_intSeparacionEntreParrafos)
    ...
    if cint(p_boleto("Operacion"))=1 or cint(p_boleto("Operacion"))=10 then
        ...
        strFormaPago = "a los  " & intDias & " días hábiles de cada fijación"        
    elseif (ucase(p_boleto("CodigoPago")) = "A") or (ucase(p_boleto("CodigoPago")) = "X") then
        ' Contra entrega de mercadería
        strFormaPago = "contra entrega de la mercadería, factura y boleto"
    ...
    elseif ucase(p_boleto("CodigoPago")="I") then
        ...
        if len(p_boleto("FechaPago"))>7 then strFormaPago = strFormaPago & " el " & left(GF_FN2DTE(p_boleto("FechaPago")),10)
        strFormaPago = strFormaPago & " contra mercadería entregada"
    else
        strFormaPago = "contra "
        select case ucase(p_boleto("CodigoPago"))            
            case "Z", "E", "K", "T", "M":             	
            	if (p_boleto("CodigoPago") <> "M") then strFormaPago = strFormaPago & "CD, "            	
            	strFormaPago = strFormaPago & "c/gtía y documentación a entera satisfacción del comprador el " & left(GF_FN2DTE(p_boleto("FechaPago")),10)            
            case "D": strFormaPago = strFormaPago & "carta de porte"
            case "R":
                strFormaPago = strFormaPago & "carta de garantía del corredor"
                if len(p_boleto("FechaPago"))>7 then strFormaPago = strFormaPago & " el " & left(GF_FN2DTE(p_boleto("FechaPago")),10)
            case "J":
                strFormaPago = strFormaPago & "carta de garantía del corredor y C/D"
                if len(p_boleto("FechaPago"))>7 then strFormaPago = strFormaPago & " el " & left(GF_FN2DTE(p_boleto("FechaPago")),10)
            case "H":
                strFormaPago = strFormaPago & "certificado de depósito"
                if len(p_boleto("FechaPago"))>7 then strFormaPago = strFormaPago & " el " & left(GF_FN2DTE(p_boleto("FechaPago")),10)
        end select
    end if
    p_intItem = p_intItem + 1
    p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos        
    strTexto = "<b>" & p_intItem & ".-</b> El pago se realizará en Buenos Aires, <b>" & strFormaPago & ", el " & p_boleto("PjeParcial") & "%</b> a la orden irrevocable de " & getDsEnterprise2(p_boleto("PagarA")) & ", menos gastos por servicios de acondicionamiento, sellados, etc.,  de corresponder, y el saldo en la liquidación final."
    p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)
    if cint(p_boleto("Operacion"))=9 then
        ' Operacion en U$S
        strTexto = "El pago se hará en pesos moneda argentina. A tal efecto, el vendedor facturará el precio en pesos, moneda argentina, considerando el tipo de cambio comprador para el Dólar estadounidense fijado por el Banco de la Nación Argentina para divisas de exportación conforme al producto objeto del contrato al cierre de las operaciones del día hábil inmediato anterior a la fecha de la facturación. El vendedor deberá emitir la factura siempre y cuando se encuentre la mercadería descargada y en un plazo no mayor a las 72 horas hábiles de producida la descarga. El saldo en pesos pendiente de cada factura y la calidad resultante, quedarán fijos y no sujetos a ajuste, actualización, interés o repotenciación alguna, de ninguna naturaleza, por ninguna causa o concepto"
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)
    elseif cint(p_boleto("Operacion"))=10 then
        ' Operacion a fijar en U$S
        strTexto = "El pago se hará en pesos, moneda argentina. A tal efecto, el vendedor facturara el precio en pesos, moneda argentina, considerando el tipo de cambio comprador para el Dólar estadounidense fijado por el Banco de la Nación Argentina para divisas de exportación conforme al producto objeto del contrato al cierre de las operaciones del día hábil inmediato anterior a la fecha de la facturación. El saldo en pesos pendiente de cada factura y la calidad resultante, quedaran fijos y no sujetos a ajuste, actualización, interés o repotenciacion alguna, de ninguna naturaleza, por ninguna causa o concepto."
        'strTexto = "El pago se hará en pesos moneda argentina. A tal efecto, el vendedor facturará el precio en pesos, moneda argentina, considerando el MAT Bs. As. del día anterior a la fecha de facturación. // Se puede fijar hasta 1/2 hora antes del cierre del MAT."
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_JUSTIFY)
    end if
end sub
'*****************************************************************************************************************
sub armarClausulaMercaderia(p_oPDF, p_boleto, p_intLineaBase, byref p_intItem, p_intAnchoLinea,p_intAltoRenglon, p_intSeparacionEntreParrafos)
    if cint(p_boleto("KCCOR")) > 0 then
        ...
        strTexto = strTexto & " " & getDsEnterprise2(p_boleto("KCCOR")) & " es intermediario obligado a actuar como agente de Ret. de Impuesto a las Gcias. en la presente operación, conforme lo dispuesto por el Art. 2º Inc. b) de la R. G. 2118/2006 y de acuerdo a lo expresado en el contrato al cual esta documentación corresponde."
        p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_LEFT)
    end if
end sub
'*****************************************************************************************************************
sub armarClausulaAFIP(p_oPDF, p_boleto, p_intLineaBase, byref p_intItem, p_intAnchoLinea,p_intAltoRenglon, p_intSeparacionEntreParrafos)    
	...
	p_intLineaBase = p_intLineaBase + p_intSeparacionEntreParrafos
	strTexto = "<b>" & p_intItem & ".-</b> "
	strTexto = strTexto & "El presente boleto será registrado ante la Afip en cumplimiento de la RG AFIP nº 2596 y sus modificatorias. Resulta condición esencial para la vigencia y perfeccionamiento de este boleto la obtención de la Constancia establecida por dicha Resolución General. Para el caso de no obtenerse la referida Constancia, el presente boleto quedará sin efecto y se considerará como no celebrado en los términos del art. 548 primera parte del Código Civil, sin obligación o responsabilidad alguna en cabeza del Comprador. Si dado el supuesto anterior, el Comprador ya hubiera recibido mercadería, entonces la pondrá a disposición del Vendedor, siendo a cargo de este último los gastos ocasionados por descarga, almacenaje, acondicionamiento y retiro de la misma."
	p_intLineaBase = GF_writeTextPlus(p_oPDF, 15, p_intLineaBase, strTexto, p_intanchoLinea, p_intAltoRenglon, PDF_ALIGN_LEFT)    
end sub
'*****************************************************************************************************************
function GF_dibujarOpcionesMercaderia(byref p_oPDF, byref p_intLineaBase)
    ...
    call GF_writeTextPlus(p_oPDF, 30, p_intLineaBase + 3, "es de propia producción", 100,15,PDF_ALIGN_LEFT)
    
    call GF_writeImage(p_oPDF, server.mapPath("images/checkbox.jpg"), 140, p_intLineaBase, 10, 10, 0)
    call GF_writeTextPlus(p_oPDF, 155, p_intLineaBase + 3, "es venta en consignación por cuenta y orden de varios comitentes",300,15,PDF_ALIGN_LEFT)
    
    call GF_writeImage(p_oPDF, server.mapPath("images/checkbox.jpg"), 420, p_intLineaBase, 10, 10, 0)
    call GF_writeTextPlus(p_oPDF, 435, p_intLineaBase + 3, "no es de su propia producción",200,15,PDF_ALIGN_LEFT)
...
sub armarPie(p_oPDF, p_boleto, p_intAnchoLinea, p_intLineaBase)
    ...
    strTexto = "La Cámara Arbitral de la " & strEntidadAux & " no intervendrá en ninguna cuestión que suscite como consecuencia del presente boleto si el mismo no está registrado en la " & strEntidadAux & ". No se admitirán en este boleto enmiendas, raspaduras ni agregados, si no están debidamente salvados. Los ejemplares en poder de las partes contratantes deberán ser exactamente iguales al ejemplar que queda registrado en la " & strEntidadAux & "."
    ...
    dim oPDF, diccBoleto
       ...
       if (GF_establecerValoresBoleto(p_intProducto, p_intSucursal, p_intOperacion, p_intNumero, p_intCosecha, diccBoleto) = true) then 
           ...
           if err.number > 0 then
            ...
            response.write "Contrato inválido"
       end if
end function
'*****************************************************************************************************************
...
'***************************************************************************************
'esta función toma la constante que recivio como parametro de marca de envio para insertar o actualizar la base de daos
...
Function GF_dibujarNormasDeBiotecnologiaSoja(p_oPDF, p_boleto, ByRef p_intLineaBase, ByRef p_intItem, p_intAnchoLinea, p_intSeparacionEntreParrafos)
    dim aux, strParrafo
        ...
        'strParrafo = "-------- Continúa en la proxima pagina --------"
        ...
        'p_intItem = p_intItem + 1
        'p_intLineaBase = p_intLineaBase +  p_intSeparacionEntreParrafos        
        'strParrafo = "<b>" & p_intItem & ".-</b> Como excepción, para las entregas de grano de soja que se realicen en la campaña 2014/2015, la cláusula precedente sólo será de aplicación a los cargamentos de grano de soja cuyo origen sea las provincias de Salta, Chaco, Jujuy, Catamarca, Tucumán, Formosa y Santiago del Estero  y los departamentos de San Justo, 9 de Julio, General Obligado, San Cristóbal, Vera y San Javier de la Provincia de Santa Fe."
...
Function writeInfo(p_oPDF, p_boleto) 'es para colza (09)
	...
	aux = "Para valores superiores a 43% a razón de 1% por cada por ciento o fracción proporcional."
	Call GF_writeTextPlus(p_oPDF, 315, 105, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Para valores inferiores a 43% a razón de 1% por cada por ciento o fracción proporcional."
	...
	aux = "Para valores superiores a 1% y hasta 1,5% a razón de 2,5% por cada por ciento o fracción proporcional.<br> <br>Para valores superiores a 1,5% a razón de 5% por cada por ciento o fracción proporcional."
	...
	if (p_boleto("Cosecha") >= 13) then
		...
		aux = "Hasta la tolerancia de recibo 4% a razón de 1% por cada por ciento o fracción proporcional.<br> <br>Para valores superiores a 4% será rechazo."
		Call GF_writeTextPlus(p_oPDF, 415, 285, aux, 130, 10, PDF_ALIGN_LEFT)
	else
		aux = "CUERPOS EXTRAÑOS "
		Call GF_writeTextPlus(p_oPDF, 40, 340, aux, 100, 10, PDF_ALIGN_CENTER)
		aux = "5 %"
		Call GF_writeTextAlign(p_oPDF, 220, 340, aux, 90, PDF_ALIGN_CENTER)
		aux = "NO CORRESPONDE"
		Call GF_writeTextPlus(p_oPDF, 310, 340, aux, 100, 50, PDF_ALIGN_CENTER)
		aux = "Hasta la tolerancia de recibo 5% a razón de 1% por cada por ciento o fracción proporcional.<br> <br>Para valores superiores a 5% a razón de 1,5% por cada por ciento o fracción proporcional."
		Call GF_writeTextPlus(p_oPDF, 415, 285, aux, 130, 10, PDF_ALIGN_LEFT)			
	end if
	'4 HUMEDAD
    call GF_setFont(p_oPDF, "Times",9,0)
	...
	aux = "Para contenidos superiores a 2% a razón de 2 puntos por cada por ciento o fracción proporcional."
	...
	aux = "Para contenidos superiores a    <br>20 a razón de 1 puntos por cada micromol en exceso."
	...
	'if ((p_boleto("Cosecha") = 13) and ((p_boleto("Operacion") = 9) or (p_boleto("Operacion") = 10))) then
	if (p_boleto("Cosecha") >= 13) then
		aux = "Cuando la mercadería exceda la base de humedad (8,5%) se acondicionará con los gastos de secada a convenir."
	else
		aux = "Cuando la mercadería exceda la base de humedad (8,5%) se descontara la merma correspondiente de acuerdo a las tablas establecidas por el IASCAV y que forman parte de la presente Norma de Clasificación."
	end if
	...
	if (p_boleto("Cosecha") >= 13) then
		...
		aux = "(4)  Libre de granos de maíz y soja."
...
Function writeInfoCebada(p_oPDF, p_boleto,p_cosecha,p_operacion) '(producto 17)
	...
	aux = "Valores inferiores al 98% y hasta 95% a razón de 1% por cada por ciento ó fracción."
	Call GF_writeTextPlus(p_oPDF, 355, 110, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Por debajo de 95%, será de rechazo."
	Call GF_writeTextPlus(p_oPDF, 455, 110, aux, 90, 10, PDF_ALIGN_LEFT)
	'2 Granos quebrados, partidos, pelados, dañados y materias extrañas
	aux = "GRANOS QUEBRADOS, PARTIDOS, PELADOS, DAÑADOS Y MATERIAS EXTRAÑAS"
	...
	if (cdbl(p_Cosecha) < 17) then
	    aux = "Para valores inferiores a 2,0% a razón de 1% por cada por ciento o fracción proporcional"
	...
	aux = "Para valores superiores a 2,0% y hasta el 4%, a razón de 1% por cada por ciento o fracción proporcional"
	Call GF_writeTextPlus(p_oPDF, 355, 170, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Por encima del 4%, la mercadería será de rechazo"
	...
	aux = "Para valores superiores a 0,20% y hasta el 0,4%,será a razón de 1% por cada por ciento o fracción proporcional"
	Call GF_writeTextPlus(p_oPDF, 355, 260, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Mercaderia que exceda el 0,4%, será de rechazo"
	...
	aux = "Para valores superiores a 0,5% y hasta el 1%, a razón de 1% por cada por ciento o fracción proporcional"
	Call GF_writeTextPlus(p_oPDF, 355, 340, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Mercaderia que exceda el 1%, será de rechazo"
	...
	if (cdbl(p_Cosecha) < 17) then
	    aux = "Para valores inferiores a 3, a razón de 1% por cada por ciento o fracción proporcional"
    else
        aux = "Sin Bonificaciones"
    end if	    
	Call GF_writeTextPlus(p_oPDF, 265, 420, aux, 80, 10, PDF_ALIGN_LEFT)
	aux = "Para valores superiores a 3,0% y hasta el 4%, a razón de 1% por cada por ciento o fracción proporcional"
	Call GF_writeTextPlus(p_oPDF, 355, 420, aux, 90, 10, PDF_ALIGN_LEFT)
	aux = "Mercaderia que exceda el 4%, será de rechazo"
	...
	aux = "Mercaderia que esté por debajo del 85%, será de rechazo"
	...
	aux = "Mínimo 9%"
	if ((cdbl(p_Cosecha) = 15) or (cdbl(p_Cosecha) = 16)) then aux = "Mínimo 9.5%"
	if (cdbl(p_Cosecha) = 17) then aux = "Mínimo 10%"
	Call GF_writeTextAlign(p_oPDF, 120, 550, aux, 60, PDF_ALIGN_CENTER)
	aux = "Máximo 12%"
	if ((cdbl(p_Cosecha) = 15) or (cdbl(p_Cosecha) = 16)) then aux = "Máximo 12.5%"	
	if (cdbl(p_Cosecha) = 17) then aux = "Máximo 12%"	
	...
	if (cdbl(p_Cosecha) < 17) then	    	
	    aux = "Para valores inferiores a 12%,a razón de 1,2% por cada por ciento o fracción proporcional"
    else
        aux = "Sin Bonificaciones"
    end if 	    
	Call GF_writeTextPlus(p_oPDF, 265, 605, aux, 85, 10, PDF_ALIGN_LEFT)
	aux = "Mercadería que exceda el 12,5% de humedad, será de rechazo"
