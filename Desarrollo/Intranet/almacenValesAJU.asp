...
<%
if (fromTC = 1) then
	submitPage = "almacenValesAJU.asp"
else
	submitPage = "almacenValesTitulo.asp?cdVAle=" & CODIGO_VS_AJUSTE_VALE
end if	
%>
<form id="frmSel" name="frmSel" action="<% =submitPage %>" method="POST">	
	<table class="reg_Header" align="center" width="70%" border="0">				
        ...
        <tr>
			<td class="reg_Header_navdos"><%= GF_TRADUCIR("Part. Pres./Sector") %></td>
			<td colspan="2">
				<%
					if vs_idObra <> 0 then
				...
				<td colspan="6">
					<table class="reg_Header" width="100%" id="tblArticulos">
						<tr class="reg_Header_nav">
							...
							<%	if (esPrestamo) then %>
								<td align="center" width="10%"><% =GF_TRADUCIR("Entregados") %></td>
								<td align="center" width="10%"><% =GF_TRADUCIR("Devueltos") %></td>
							<%	else	%>
