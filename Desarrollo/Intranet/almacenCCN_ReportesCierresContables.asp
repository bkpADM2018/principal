...
dim idAlmacen, rs, idArticulo, dsArticulo, cdCategoria, dsCategoria, myBodyOnLoad
...
<script type="text/javascript">
	...
	function bodyOnLoad() {
		...
		tb.draw();		
		if ('<%=idDivision%>'>0){realizarConsulta('<%=idDivision%>', '<%=txtFechaCierre%>');}
	...
	function realizarConsulta(division, fecha){
		document.getElementById("results").innerHTML = ""; 
		document.getElementById("imgLoading").style.position = "relative";
		document.getElementById("imgLoading").style.visibility  = "visible";
		ch.bind("almacenCCN_ReportesCierresContablesAjax.asp?idDivision=" + division + "&fecha=" + fecha, "realizarConsultaCallback()");
		ch.send();			
	}
	...
	function armarPDF(division, fecha){
		window.open("almacenCCN_ReporteCtaPrint.asp?idDivision=" + division + "&fechaCierre=" + fecha);
		}
</script>
</head>
<body onLoad="bodyOnLoad()">
<% call GF_TITULO2("kogge64.gif","Administración Almacenes - Reporte Por Cuentas") %>
<div id="toolbar"></div>
<form id="frmSel">
