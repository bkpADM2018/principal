...
if idContrato <> 0 then
	...
	CTC_ImporteAsignado = getCTCTotalAsignado(idContrato)
else
...
if Controlar then
	if accion = ACCION_GRABAR then
		...
		strSQL = "SELECT * FROM TBLOBRACTCAJUSTES WHERE IDCONTRATO=" & idContrato & " AND APLICADO='" & TIPO_NEGACION & "' AND TIPOAJUSTE='" & CTC_AJUSTE_GENERAL & "'"  				
		...
		if not rs.eof then
			...
			strSQL = "INSERT INTO TBLOBRACTCAJUSTES(IDCONTRATO, IMPORTEPESOS, IMPORTEDOLARES, OBSERVACIONES, APLICADO, CDUSUARIO, MOMENTO, TIPOAJUSTE) VALUES(" & idContrato & "," & ajuPesos & "," & ajuDolares & ", '" & CTC_observaciones & "','" & TIPO_NEGACION & "','" & session("usuario") & "'," & session("MmtoSistema") & ", '" & CTC_AJUSTE_GENERAL & "')"				
		...
		if (CDbl(ajusteImporte) < 0)  then					            
			...
			strSQL="Update TBLOBRACONTRATOS set IMPORTEPESOS=" & newTotalPesos & ", IMPORTEDOLARES=" & newTotalDolares & ", ESTADO=" & myEstado & " where IDCONTRATO=" & idContrato
			Call executeQueryDb(DBSITE_SQL_INTRA, rs, "UPDATE", strSQL)
...
function Controlar
	if accion = "" then 
	    ...
	    if (Round(cdbl(newTotal)*100, 0) < Round(cdbl(CTC_ImporteAsignado), 0)) then Call setError(CTZ_AJU_TOTAL_BAJO)
...
<table class="reg_header" align="center" width="95%" border="0">				
	...
	<tr>
		<td class="reg_header_nav recuadroRound"><% =GF_TRADUCIR("Asignado hasta el momento") %></td>						
		<td align="right"><font size="+1"><b><%=getSimboloMoneda(CTC_cdMoneda) & " " & GF_EDIT_DECIMALS(CTC_ImporteAsignado,2)%></b></font></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td align="right"><font size="+1"><b><%=getSimboloMoneda(CTC_cdMoneda) & " " & GF_EDIT_DECIMALS(saldo,2)%></b></font></td>	
	</tr>
	<tr>
		<td COLSPAN="3" align="right"><HR></td>	
	</tr>
