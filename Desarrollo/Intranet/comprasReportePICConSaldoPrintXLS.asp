...
Function drawDetalle()
	...
	strSQL = "SELECT TSALDO.*,    "&_
			 ...
			 "       	       IDDIVISION,    "&_
			 "        	       MONEDAPIC    "&_
			 "             FROM   VWCTZCABECERABIENESACTIVOS    "&_
			 "          ) AS TSALDO    "&_
			 " INNER JOIN (SELECT IDCOTIZACION, CDUSUARIO FROM TBLCTZFIRMAS CTZF WHERE SECUENCIA = (SELECT MIN(SECUENCIA) FROM TBLCTZFIRMAS WHERE IDCOTIZACION= CTZF.IDCOTIZACION  group by IDCOTIZACION)) FIRMA   "&_
			 "               ON FIRMA.IDCOTIZACION = TSALDO.IDCOTIZACION    "&_
			 " LEFT JOIN [Database].[dbo].MET001A EMP    "&_
			 "               ON TSALDO.idproveedor = EMP.NROEMP "&_
			 " LEFT JOIN tbldatosobras OBR    "&_
