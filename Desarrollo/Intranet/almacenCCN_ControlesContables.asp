...
<script type="text/javascript">
	...
	function realizarConsulta(division, almacen, fecha){
		...
		ch.bind("almacenCCN_ControlesContablesAjax.asp?idDivision=" + division + "&idAlmacen=" + almacen + "&fecha=" + fecha, "realizarConsultaCallback()");
		ch.send();			
	}
	...
	function verArtSinPrecio(pFecha, pIdCierre){
		ch.bind("almacenCCN_ControlesContablesAjax.asp?idAlmacen=<%=idAlmacen%>&idDivision=<%=idDivision%>&fecha=" + pFecha, "realizarConsultaCallback()");
		ch.send();			
	}
</script>
</head>
<body onLoad="bodyOnLoad()">
<% call GF_TITULO2("kogge64.gif","Administración Almacenes - Articulos sin precio") %>
<div id="toolbar"></div>
<form id="frmSel">
