...
dim strSQL, rs2, con2, sqlINS, rsINS, conINS
	...
	if rs2.eof then
		'Response.Write "<br>Valores (" & pIdAlmacen & ") (" & pidArticulo & ") " & cdbl(pExistencia) & "," & pSobrante
		...
		'Response.Write "<br>Valores (" & pIdAlmacen & ") (" & pidArticulo & ") " & cdbl(pExistencia) & "," & cdbl(rs2("EXISTENCIA")) & " Sum(" & cdbl(pExistencia) + cdbl(rs2("EXISTENCIA")) & "), sob (" & cdbl(pSobrante) & "," & cdbl(rs2("SOBRANTE")) & ") sum(" & cdbl(pSobrante) + cdbl(rs2("SOBRANTE")) & ")"
	...
	'Response.End 
	if len(sqlINS)>0 then call executeQueryDB(DBSITE_SQL_INTRA, rsINS, "EXEC", sqlINS)
end sub
'------------------------------------------------------------------------------------------------------------
