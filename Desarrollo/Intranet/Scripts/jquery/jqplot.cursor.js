...
(function($) {
    ...
    $.jqplot.Cursor.prototype.doZoom = function (gridpos, datapos, plot, cursor) {
		...
		var monthsCount,milisecs,secs,mins,hours,days;
		milisecs = 1000;
		secs     = 60;
		mins     = 60;
		hours    = 24;
                ...
                for (var ax in datapos) {
                    ...
                    if ((c.constrainZoomTo == 'none') || (c.constrainZoomTo == 'x' && ax.charAt(0) == 'x') || (c.constrainZoomTo == 'y' && ax.charAt(0) == 'y')) {   
                        ...
                        if (dp != null) {           
							...
							if (ax.charAt(0)=='x') {
								monthsCount = Math.round((((((axes[ax].max-axes[ax].min)/milisecs)/secs)/mins)/hours)/days)+1;
								if ( monthsCount == 1){
									axes[ax].numberTicks = 2; // minimal number of ticks
								}else{
									if (monthsCount > 12 ){
										axes[ax].tickInterval = monthsCount/12 + ' months';
									}else{
										axes[ax].numberTicks = monthsCount;
									}
								}
							}
                            axes[ax]._ticks = [];
