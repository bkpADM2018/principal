...
Function armarInfoPrincipal(p_partida, p_pedido, p_Proveedor, p_division, p_idContrato)
	...
	Call GF_writeTextAlign(oPDF,345, 102, GF_TRADUCIR("División"), 75, PDF_ALIGN_LEFT)
	...
	Call GF_writeTextAlign(oPDF,55, py, GF_TRADUCIR("Descripción"), 295, PDF_ALIGN_LEFT)
...
Function printMoneyLimitWarning(ByRef y_posicion, ByRef nroPagina, ByRef regCargados)
	...
	if ((PIC_idPedido = 0) and (PIC_idContrato = 0)) then
        ...
        Call GF_writeTextAlign(oPDF,100,y_posicion + 5, GF_TRADUCIR("TOTAL Compras Directas realizadas en los últimos 30 días (" & cantPICDirectos30 - 1 & " Pics): "), 50, PDF_ALIGN_RIGHT)            
        ...
        if (regCargados >= PIC_MAXREGISTROS) then Call addNewPage(y_posicion, nroPagina)
        Call GF_writeTextAlign(oPDF,100,y_posicion + 5, GF_TRADUCIR("TOTAL Compras Directas realizadas en los últimos 12 meses (" & cantPICDirectos365 - 1 & " Pics):"), 50, PDF_ALIGN_RIGHT)                              
...
Function get_PageWrite(nroPagina)
	...
	Call GF_setFont(oPDF,"ARIAL",8,0)
	Call GF_writeTextAlign(oPDF,530,835, GF_TRADUCIR("Página Nº " & nroPagina), 50, PDF_ALIGN_RIGHT)
	Call GF_writeTextAlign(oPDF,10,835, GF_TRADUCIR("Cargó: " & PIC_usuario & " - " & GF_FN2DTE(PIC_momento) &" - Fecha de impresión: " & GF_FN2DTE(session("MmtoSistema"))), 50, PDF_ALIGN_LEFT)    
end Function
'-----------------------------------------------------------------------------------------------
...
Function get_PageWriteAjustes(nroPagina, pCdUsuario, pMomento)
	...
	Call GF_setFont(oPDF,"ARIAL",8,0)
	Call GF_writeTextAlign(oPDF,500,835, GF_TRADUCIR("Página De Ajuste Nº " & nroPagina), 50, PDF_ALIGN_RIGHT)
	Call GF_writeTextAlign(oPDF,10,835, GF_TRADUCIR("Cargó: " & PIC_usuario & " - " & GF_FN2DTE(pMomento)) &" - Fecha de impresión: " & GF_FN2DTE(session("MmtoSistema")), 50, PDF_ALIGN_LEFT)    
end Function
'-----------------------------------------------------------------------------------------------
...
Function escribeRegistroAjuste(p_y, pAjuste, p_codigo, p_descripcion, p_Moneda, p_importe, pIdArea, pIdDetalle, pObservaciones, pUsuario, pMomento)
	...
	if (PIC_firmante1Cd <> "") then 
        ...
        Call GF_writeTextAlign(oPDF,101 + 40, p_y,"No requiere autorización por ser un ajuste que reduce el monto autorizado.", 200, PDF_ALIGN_LEFT)
    end if	    
	p_y = p_y + 10
	Call GF_squareBox(oPDF, 100 + 40, p_y, 380, 10, 0, "#ffffff", "#000000", 1, PDF_SQUARE_NORMAL)
