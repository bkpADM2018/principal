...
End function
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Function corteControlProdcutoMermaVolatil(p_Rs,p_Producto)
    corteControlProdcutoMermaVolatil = false
    if (not p_Rs.Eof) then
        if (Cdbl(p_Producto) = Cdbl(p_Rs("CDPRODUCTO"))) then corteControlProdcutoMermaVolatil = true
    end if
End function
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Function corteControlClienteMermaVolatil(p_Rs,p_Cliente,p_Producto)
    corteControlClienteMermaVolatil = false
    if (not p_Rs.Eof) then
        if ((Cdbl(p_Cliente) = Cdbl(p_Rs("IDORIGEN"))) and (Cdbl(p_Producto) = Cdbl(p_Rs("CDPRODUCTO")))) then corteControlClienteMermaVolatil = true
    end if
End function
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Function writeObservationsMermaVolatil(pto, ByRef pRs, ByRef py)
    ...
    while not pRs.Eof
        ...
        'while (corteControlFechaMermaVolatil(pRs, auxFecha))
            ...
            while (corteControlProdcutoMermaVolatil(pRs, auxCdProducto))
                auxCdCliente = Cdbl(pRs("IDORIGEN"))
                totalKilos = 0
                while (corteControlClienteMermaVolatil(pRs, auxCdCliente, auxCdProducto))
                    totalKilos = Cdbl(totalKilos) + ABS(Cdbl(pRs("KILOSAJUSTE")))
                    pRs.MoveNext()
                wend
                py = py + 12
                if (CDbl(py) > PAGE_HEIGHT_SIZE) then py = nuevaHoja()
                Call GF_writeTextAlign(oPDF, 90, py, Left(getDsCliente(auxCdCliente), 25) , 200, PDF_ALIGN_LEFT)
...
Function writeObservationsDraft(pto, pIdOrigen, ByRef py)
	...
	if not rs.EoF then
		...
		py = py + SEPARACION_Y
		Call GF_writeTextAlign(oPDF, 135, py,GF_TRADUCIR("Cargas de Toepfer:"), 100, PDF_ALIGN_RIGHT)
		Call GF_setFont(oPDF,"ARIAL",8,0)			
...
Function writeSignatureMermaVolatil(pPto, pIdAjuste)
    Dim strSQL, rs,cdGerente, dsGerente, firmaGerente, cdController, dsController, firmaController, cdDirector, dsDirector, firmaDirector, cdMercaderia, dsMercaderia, firmaMercaderia,rsEst
    ...
    if not rs.Eof then
        cdMercaderia = rs("CDUSUARIO")
		dsMercaderia = getUserDescription(cdMercaderia)
    ...
    if (firmaMercaderia <> "") then
	    firma = obtenerFirma(cdMercaderia)		
	...
	if (dsMercaderia <> "") then Call GF_writeTextAlign(oPDF,296, 688, dsMercaderia, 286, PDF_ALIGN_CENTER)
...
Function drawBoxSignature()
    pY = 706
	Call GF_squareBox(oPDF, 10, pY, 190, 15, 0, "#517b4a", "#000000", 1, PDF_SQUARE_NORMAL)
    Call GF_squareBox(oPDF, 200, pY, 190, 15, 0, "#517b4a", "#000000", 1, PDF_SQUARE_NORMAL)
    Call GF_squareBox(oPDF, 390, pY, 190, 15, 0, "#517b4a", "#000000", 1, PDF_SQUARE_NORMAL)
...
Function drawBoxSignatureMermaVolatil()
    pY = 595
	Call GF_squareBox(oPDF, 10,  pY, 286, 15, 0, "#517b4a", "#000000", 1, PDF_SQUARE_NORMAL)
    Call GF_squareBox(oPDF, 296, pY, 286, 15, 0, "#517b4a", "#000000", 1, PDF_SQUARE_NORMAL)
    pY = pY + 3
	Call GF_setFont(oPDF,"ARIAL",8,8)
	Call GF_setFontColor("#FFFFFF")
    Call GF_writeTextAlign(oPDF,10, pY,  GF_TRADUCIR("Aprobación Gerente de Puertos"),   	      286, PDF_ALIGN_CENTER)
	Call GF_writeTextAlign(oPDF,296, pY, GF_TRADUCIR("Aprobación Gerente de Mercaderias"),    286, PDF_ALIGN_CENTER)
    pY = pY + 12
	...
	Call GF_writeTextAlign(oPDF,296, pY, GF_TRADUCIR("Aprobación Directo"),    286, PDF_ALIGN_CENTER)
    pY = pY + 12
    Call GF_squareBox(oPDF, 10,  pY, 286, 100, 0, "", "#000000", 1, PDF_SQUARE_NORMAL)
	Call GF_squareBox(oPDF, 296, pY, 286, 100, 0, "", "#000000", 1, PDF_SQUARE_NORMAL)
...
End Function
'------------------------------------------------------------------------------------------------------
Function armadoPDF(pto, pIdAjuste,pMode)
    ...
    strSQL = "SELECT CASE WHEN A.FECHADESDE IS NULL THEN 0 ELSE A.FECHADESDE END AS FECHADESDE, "&_
             ...
             " WHERE  A.IDAJUSTE = " & pIdAjuste &_
			 " ORDER  BY CDPRODUCTO, IDORIGEN "	
    call executeQueryDb(pto, rs, "OPEN",strSQL)
...
Function drawHead(pto, pIdAjuste, pCdAjuste, pFechaHasta, pFechaDesde, pCdProducto, pDsProducto, pKilosAjuste, pOrigen)
	...
	if (pIdAjuste > 0) then Call GF_squareBox(oPDF, 500, py , 30 , 18, 0, "#F4F4F4", "#000000", 1, PDF_SQUARE_NORMAL)				
    py = py + 18
	Call GF_squareBox(oPDF, 10 , py, 100, 18, 0, "#F4F4F4", "#000000", 1, PDF_SQUARE_NORMAL)
	Call GF_squareBox(oPDF, 110, py, 470, 18, 0, "#FFFFFF", "#000000", 1, PDF_SQUARE_NORMAL)
    py = py + 18
	...
	if (pIdAjuste > 0) then Call GF_writeTextAlign(oPDF,510, py, GF_TRADUCIR("N°:")	,  30, PDF_ALIGN_LEFT)
    py = py + 18
	...
	if (pIdAjuste > 0) then Call GF_writeTextAlign(oPDF, 535, py, pIdAjuste	,  50, PDF_ALIGN_LEFT)	
    py = py + 18
...
'**************************************** COMIENZO DE PAGINA ******************************************
'******************************************************************************************************
Dim px, idAjuste,g_Puerto, nroPagina, oPDF, pathImg,cdCliente,cdProducto
...
if (LCase(request.servervariables("script_name")) = "/intranet/poseidonajusteautorizacionprint.asp") then
	Call armadoPDF(g_Puerto,idAjuste,PDF_STREAM_MODE)
end if
%>