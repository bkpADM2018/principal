...
'-----------------------------------------------
'Function isCTCExpired : Comprueba si el contrato está vencido o no
'			True  = Vencido
'			False = Vigente
...
Function enviarMailResponsableCTC(p_idContrato, p_CdResponsableNew, p_CdResponsableOld, p_CdContrato)
	...
	mailMsj = mailMsj & "Usuario que modificó: " & session("Usuario") & " - " & getUserDescription(session("Usuario"))
    Call GP_ENVIAR_MAIL("Sistema de Compras Web - Modificación del contrato: " & p_CdContrato, mailMsj, obtenerMail(CD_TOEPFER), destino)
End Function
'------------------------------------------------------------------------------------------------------------
...
if(accion = ACCION_GRABAR)then	    
    ...
    if((Cdbl(Left(session("MmtoDato"), 8)) <= Cdbl(fechaVto))or(fechaVto = 0))then
	    ...
	    Call GP_ENVIAR_MAIL("Sistema de Compras Web - Modificación del contrato: " & CTC_cdContrato, mailMsj, obtenerMail(CD_TOEPFER), mailTo)	    
    else
	...
	<form id="frmCTC" name="frmCTC">
		...
		<table id="TBL_CTC_PAGOS" class="reg_Header" align="center" width="90%">
			...
			<tr class="reg_Header_nav">
				...
				<td width="25%"><% =GF_TRADUCIR("Descripción") %></td>
...
<%			CTC_Total_ImporteObra		= 0
			...
			While (not rsPagos.eof)
				...
				'Analiza todos los registros del PIC antes de imprimir para saber si tiene más de una minuta.					
				While ((not rsPagos.eof) and (idPIC_old = idPIC))
					...
					While ((not rsPagos.eof) and (idPIC_old = idPIC) and (idArea = idArea_old) and (idDetalle = idDetalle_old))	
						...
						'Recorro todas las facruras asociadas al PIC, si no hay facturas, habrá una sola linea con el ID de Factura NULL y se tomaran los importes del PIC.		
