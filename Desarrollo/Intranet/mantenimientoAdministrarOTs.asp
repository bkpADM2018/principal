		...
		<div id="searchfilter" class="tableasidecontent">
	        ...
	        <div class="col16">
				<select name="txtTipoOrden" id="txtTipoOrden">
					...
					<option value="<%=ORDER_TYPE_MECHANICAL%>"	<%if txtTipoOrden=ORDER_TYPE_MECHANICAL then Response.Write "Selected"%>><%=GF_Traducir("Mecánica")%></option>
...
<table class="datagrid" width="90%" align="center">
	...
	while ((not rs.eof)	and (reg < mostrar))
		...
		<tr>
			...
			<%	
				verPlanificacion = ""
				if not isNull(rs("IDOTPLAN")) then verPlanificacion = "<img src='images/calendar-16.png' style='cursor: pointer' title='Ir a planificación' onClick=verPlanificacionOT('" & rs("IDORDER") & "')>"
