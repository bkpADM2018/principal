...
Function checkOperacion(pCierre, pSecuencia)			
	...
	if pSecuencia = FIRMA_ROL_RESP_PUERTO then
		strSQL = "Select * from TBLCIERRESCABECERA where IDCIERRE=" & pCierre 
...
Function registrarFirma(pCierre, pSecuencia, llave)
	...
	strSQL="Update TBLCIERRESFIRMAS SET FECHAFIRMA=" & session("MmtoDato") & ", HKEY='" & llave & "', CDUSUARIO='" & UCASE(session("Usuario")) & "' where IDCIERRE=" & pCierre & " and SECUENCIA=" & pSecuencia	
